...
REPORT_OBJECTS_FILE = PYTEST_TUI_FILES_DIR / "report_objects.bin"
TEST_TUI_RESULT_OBJECTS_FILE = PYTEST_TUI_FILES_DIR / "test_tui_result_objects.bin"
MARKED_TERMINAL_OUTPUT_FILE = PYTEST_TUI_FILES_DIR / "marked_output.bin"
UNMARKED_TERMINAL_OUTPUT_FILE = PYTEST_TUI_FILES_DIR / "unmarked_output.bin"
...
class Results:
    ...
    def _get_unmarked_output(
        self, unmarked_file_path: Path = UNMARKED_TERMINAL_OUTPUT_FILE
    ) -> list:
    ...
    def _deduplicate_tui_test_results(self, list) -> list:
        """
        Remove duplicate TuiTestResult objects.
        """
    ...
    def _process_reports(self) -> list:
        ...
        test_infos = []
        for report in self._unpickle_test_reports():
            test_info = TestInfo()
    ...
    def _unpickle_test_reports(self):
        """Unpack pickled Pytest TestReport objects from file"""
        try:
            with open(REPORT_OBJECTS_FILE, "rb") as rfile:
                return pickle.load(rfile)
        except FileNotFoundError as e:
            raise FileNotFoundError(
                f"Cannot find {REPORT_OBJECTS_FILE}. Have you run pytest with the '--tui' option yet?"
            ) from e
...
class MarkedSections:
    ...
    def __init__(
        self, Sections: dict, marked_file_path: Path = MARKED_TERMINAL_OUTPUT_FILE
    ) -> None:
    ...
    def _get_marked_lines(
        self, marked_file_path: Path = MARKED_TERMINAL_OUTPUT_FILE
    ) -> list:
        """Return a list of all lines from the marked output file"""
        try:
            with open(MARKED_TERMINAL_OUTPUT_FILE, "r") as mfile:
                return mfile.readlines()
        except FileNotFoundError as e:
            raise FileNotFoundError(
                f"Cannot find {MARKED_TERMINAL_OUTPUT_FILE}. Have you run pytest with the '--tui' option yet?"
            ) from e
